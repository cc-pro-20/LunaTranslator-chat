...
from myutils.utils import stringfyerror, PriorityQueue, set_lang_override, clear_lang_override
...
class basetrans(commonbase):
    ...
    def gettask(self, content):
        # fmt: off
        callback, contentsolved, callback, is_auto_run, optimization_params, manual_lang = content
        # fmt: on
        if callback:
            priority = 1
        else:
    ...
    def translate_and_collect(
        self, tgtlang_1, contentsolved: "GptTextWithDict|str", is_auto_run, callback, manual_lang=None
    ):
        if manual_lang:
            set_lang_override(*manual_lang)
        try:
    ...
    def _fythread(self):
        ...
        while self.using:
            ...
            if content is None:
                break
            # fmt: off
            callback, contentsolved, waitforresultcallback, is_auto_run, optimization_params, manual_lang = content
            # fmt: on
