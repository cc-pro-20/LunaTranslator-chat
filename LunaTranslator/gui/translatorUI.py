...
from gui.manualtranslate import ManualTranslatePanel
...
class TranslatorWindow(resizableframeless):
    ...
    def addbuttons(self):
        ...
        functions = (
            ...
            (
                "manualtranslate",
                buttonfunctions(
                    clicked=self.toggle_manual_panel,
                    colorstate=lambda: globalconfig.get(
    ...
    def changeextendstated(self):
        if not hasattr(self, "manual_panel"):
            return
        dh = self.dynamicextraheight()
        toolbar_h = self.dynamictoolbarheight()
        panel_h = dh - toolbar_h
        if globalconfig.get("verticalhorizontal", False):
            self.translate_text.move(0, 0)
            text_w = self.width() - dh
            text_h = self.height() - panel_h
            self.translate_text.resize(int(text_w), max(int(text_h), 1))
            if panel_h:
                self.manual_panel.setGeometry(
                    0, self.height() - panel_h, self.width(), panel_h
                )
        else:
            if panel_h:
                self.manual_panel.setGeometry(0, toolbar_h, self.width(), panel_h)
            self.translate_text.move(0, dh)
            text_h = self.height() - dh
            self.translate_text.resize(self.width(), max(int(text_h), 1))
    ...
    def __init__(self):
        ...
        if self.width() < minw or self.height() < minh:
            self.resize(max(self.width(), minw), max(self.height(), minh))
        ...
        self.manual_panel = ManualTranslatePanel(self)
        self.manual_panel.translate_requested.connect(self.do_manual_translate)
        self.manual_panel.setVisible(globalconfig.get("manual_panel_visible", False))
    ...
    def toggle_manual_panel(self):
        visible = not globalconfig.get("manual_panel_visible", False)
        globalconfig["manual_panel_visible"] = visible
        panel_h = globalconfig.get("manual_panel_height", 140)
        if visible:
            self.resize(self.width(), self.height() + panel_h)
        else:
            self.resize(self.width(), max(self.height() - panel_h, 100))
        self.manual_panel.setVisible(visible)
        if visible:
            self.manual_panel.text_input.setFocus()
        self.changeextendstated()
        self.refreshtoolicon()

    def do_manual_translate(self, text):
        old_showfanyi = globalconfig.get("showfanyi", True)
        globalconfig["showfanyi"] = True
        gobject.base.textgetmethod(text, is_auto_run=False, use_manual_lang=True)
        if hasattr(self, "_restore_lang_timer") and self._restore_lang_timer:
            self._restore_lang_timer.stop()
        self._restore_lang_timer = QTimer(self)
        self._restore_lang_timer.setSingleShot(True)
        self._restore_lang_timer.timeout.connect(
            lambda: self._restore_manual_langs(old_showfanyi)
        )
        self._restore_lang_timer.start(15000)

    def _restore_manual_langs(self, showfanyi):
        globalconfig["showfanyi"] = showfanyi

    def dynamicextraheight(self):

        base = 0
        if self.radiu_valid or globalconfig["locktools"]:
            if globalconfig.get("verticalhorizontal", False):
                base = int(IconLabelX.w())
            else:
                base = int(IconLabelX.h())

        if hasattr(self, "manual_panel") and self.manual_panel.isVisible():
            base += globalconfig.get("manual_panel_height", 140)
        return base

    def dynamictoolbarheight(self):
        if self.radiu_valid or globalconfig["locktools"]:
            if globalconfig.get("verticalhorizontal", False):
    ...
    def resizeEvent(self, e: QResizeEvent):
        ...
        toolbar_h = self.dynamictoolbarheight()
        panel_h = wh - toolbar_h

        if globalconfig.get("verticalhorizontal", False):
            height = self.width() - wh
            th = self.height() - panel_h
            self.translate_text.resize(int(height), max(int(th), 1))
            self.translate_text.move(0, 0)
            ...
            self.translate_text.resize(self.width(), max(int(height), 1))
