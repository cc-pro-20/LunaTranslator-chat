...
import socket, gobject, uuid, functools, threading
...
def set_lang_override(srclang, tgtlang):
    _lang_override.src = srclang
    _lang_override.tgt = tgtlang


def clear_lang_override():
    _lang_override.src = None
    _lang_override.tgt = None
...
def getlangsrc() -> Languages:
    src = getattr(_lang_override, "src", None)
    if src is not None:
        return __fucklang(True, (src, src))
    return __fucklang(True, __internal__getlang("private_srclang_2", "srclang4"))


def getlangtgt() -> Languages:
    tgt = getattr(_lang_override, "tgt", None)
    if tgt is not None:
        return __fucklang(False, (tgt, tgt))
